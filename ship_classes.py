...
class CurrentBoiler:
    def __init__(self, boiler_load, steam_pressure, fuel,
                 feed_water_temp, feed_pump_pressure, fuel_pressure,
    ...
    def reload(self):
        ...
        pls_min = [1, -1]
        self.boiler_load += int(random.choice(prct_list) * 0.01 * random.choice(pls_min) * self.boiler_load)
        self.fuel += random.choice(prct_list) * 0.01 * random.choice(pls_min) * self.fuel
        self.feed_pump_pressure += random.choice(prct_list) * 0.01 * random.choice(pls_min) * self.feed_pump_pressure  #
        self.fuel_pressure += random.choice(prct_list) * 0.01 * random.choice(pls_min) * self.fuel_pressure  #
        self.circulatory_pump_pressure += random.choice(prct_list) * 0.01 * random.choice(pls_min) * self.circulatory_pump_pressure


class Boiler:
    def __init__(self, steamcapacity, boiler_number, steam_pressure,
                 temp_feed_water, temp_air,
                 fuel_consumption, excess_air_ratio,
                 oxygen_content, air_consumption, gas_flow,
                 combustion_pressure, fuel,
                 classification
                 ):
...
class CurrentGenerator:
    def __init__(self, amperage, fuel, temp, exit_gas_temp, average_temp_exit,
                 compression_pressure, combustion_pressure,
                 cold_water_of, cold_water_exit, pressure,
                 consump_water, temp_cold_water_in,
                 temp_cold_water_out):
...
self.temp_cold_water_out,)
    def reload(self):
        prct_list = list(range(1, 25, 1))
        pls_min = [1, -1]
        self.amperage += int(random.choice(prct_list)* 0.01 * random.choice(pls_min) * self.amperage)
        self.fuel += int(random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.fuel)
        self.temp += random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.temp
        self.exit_gas_temp = list(map(lambda x: x+random.choice(prct_list)* 0.01 * random.choice(pls_min)* x, self.exit_gas_temp )) #random.choice(prct_list)* 0.01 * random.choice(pls_min)* self.exit_gas_temp
        self.average_temp_exit+= random.choice(prct_list)* 0.01 * random.choice(pls_min)*self.average_temp_exit #
        self.combustion_pressure = list(map(lambda x: x+random.choice(prct_list)* 0.01 * random.choice(pls_min)* x, self.combustion_pressure ))
        self.cold_water_of+= random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.cold_water_of
        self.cold_water_exit+= random.choice(prct_list)* 0.01 * random.choice(pls_min)*self.cold_water_exit #
        self.pressure+= random.choice(prct_list)* 0.01 * random.choice(pls_min)*self.pressure #
        self.consump_water+= random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.consump_water
        self.temp_cold_water_in+= random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.temp_cold_water_in
        self.temp_cold_water_out+= random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.temp_cold_water_out


class Generator:
    def __init__(self, model, power, voltage, amperage, frequency, efficiency, cos,):
        self.model = model
        self.power = power
...
class Diesel:
    def __init__(self, model, cylinder_diameter, piston_stroke,
                 power, rpm, average_pressure, piston_speed,
                 combustion_pressure, pisto_order, start_air_pressure,
                 weight, compression_ratio, press_principle,
                 normal_start_air_pressure, minimal_start_air_pressure,
                 regulator):
        self.model                           = model
...
class Ship:
    def __init__(self, name, flag, number, class_, body_type,
                main_engine, max_speed, developer,
                build_date, displacement, deadweight,
                lenght, width,
        max_height, boart_height, cargo_tanks):
        self.name=name
        self.flag=flag
        self.number=number
        self.class_=class_
...
class CurrentEngine():
    def __init__(self, engine_speed, speed_tc, water_engine_inlect, water_charge_air, control_air, lube_oil_engine, nozzle_cool_water,
                 charge_air, starting_air, lube_oil_tc, fuel_oil):
...
self.fuel_oil  ,)
    def reload(self):
        ...
        pls_min = [1, -1]
        self.engine_speed += int(random.choice(prct_list)* 0.01 * random.choice(pls_min) * self.engine_speed)
        self.speed_tc += int(random.choice(prct_list)* 0.01 * random.choice(pls_min) *self.speed_tc)
        self.water_engine_inlect += random.choice(prct_list2)* 0.01 * random.choice(pls_min) *self.water_engine_inlect
        self.water_charge_air  += random.choice(prct_list2)* 0.01 * random.choice(pls_min)* self.water_charge_air
        self.control_air+= random.choice(prct_list)* 0.01 * random.choice(pls_min)*self.control_air #
        self.lube_oil_engine+= random.choice(prct_list)* 0.01 * random.choice(pls_min)* self.lube_oil_engine#
        self.nozzle_cool_water+= random.choice(prct_list2)* 0.01 * random.choice(pls_min)*self.nozzle_cool_water
        self.charge_air+= random.choice(prct_list2)* 0.01 * random.choice(pls_min) *self.charge_air
        self.starting_air+= random.choice(prct_list)* 0.01 * random.choice(pls_min)*self.starting_air #
        self.lube_oil_tc+= random.choice(prct_list)* 0.01 * random.choice(pls_min)*self.lube_oil_tc #
        self.fuel_oil+= random.choice(prct_list2)* 0.01 * random.choice(pls_min) *self.fuel_oil
class Engine:
    def __init__(self, type_, cylinder_number, piston_diameter,
                piston_stroke, max_power, max_rpm,
                nominal_power, nominal_rpm, max_pressure,
                jacket_cooling, launch_system,
                piston_cooling,
                purge_air_cooler_cooling, cylinder_order,
                fresh_water_temp, fuel_consumption,
...
class CurrentInfo:
    def __init__(self, depature_port, arrival_port, ATD, ETA, Vessel_Local_Time,
...
class CrewMember:
    def __init__(self, name, position, current_location, current_occupation, picture_url):
        self.name = name
...
Текущее занятие: {3}'''.format(self.position,
self.name,
...
class Cargo:
    def __init__(self, volume, fuel_volume, fuel, fuel_weight,):
        self.volume = volume
...
Груз: {1}
Объем груза: {2} м^3
Масса груза: {3} т.'''.format(self.volume,
self.fuel_volume,
self.fuel,
self.fuel_weight,
)